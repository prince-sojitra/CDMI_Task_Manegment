...
const updateCardOrder = async (boardId, sourceId, destinationId, destinationIndex, cardId, user, callback) => {
	try {
		...
		if (!validate || !validate2) return callback({ message: 'List or board informations are wrong' });
		...
		if (!validate) return callback({ message: 'List or card informations are wrong' });
