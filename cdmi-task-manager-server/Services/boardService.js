...
const removeMember = async (id, userId, callback) => {
	try {
		// Get board by id
		const board = await boardModel.findById(id);

		// Set variables
		board.members = board.members.filter((el) => el._id.toString() !== userId.toString());
		// Save changes
		await board.save();
...
const getBoardMembers = async (id, callback) => {
	try {
		// Get board by id
		const board = await boardModel.findById(id);
		return callback(false, board.members.map((el) => el.email));
	} catch (error) {
		return callback({ message: 'Something went wrong', details: error.message });
	}
};

const getMembers = async (callback) => {
	try {
		// Get board by id
		const board = await userModel.find().select('name surname email');
		return callback(false, board);
	} catch (error) {
		return callback({ message: 'Something went wrong', details: error.message });
	}
