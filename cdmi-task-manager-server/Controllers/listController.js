...
const create = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ message: 'You can not add a list to the board, you are not a member or owner!' });

	// Call the service to add new list
	await listService.create({ title: title, owner: boardId, createdBy: req.user._id }, req.user, (err, result) => {
...
const getAll = async (req, res) => {
	...
	const validate = req.user.boards.filter((board) => board === boardId);
	if (!validate)
		return res.status(400).send({ message: 'You cannot get lists, because you are not owner of this lists!' });
...
const updateCardOrder = async (req, res) => {
	...
	if (!validate) return res.status(403).send({ message: 'You cannot edit the board that you hasnt' });
