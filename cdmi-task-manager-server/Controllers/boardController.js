...
const create = async (req, res) => {
	...
	if (!(title && backgroundImageLink))
		return res.status(400).send({ message: 'Title and/or image cannot be null' });
...
const getById = async (req, res) => {
	...
	if (!validate)
		return res.status(400).send({ message: 'You can not show the this board, you are not a member or owner!' });
...
const updateBoardTitle = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ message: 'You can not change title of this board, you are not a member or owner!' });
...
const updateBoardDescription = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ message: 'You can not change description of this board, you are not a member or owner!' });
	const { boardId } = req.params;
...
const updateBackground = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ message: 'You can not change background of this board, you are not a member or owner!' });
	const { boardId } = req.params;
...
const addMember = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ message: 'You can not add member to this board, you are not a member or owner!' });
	const { boardId } = req.params;
...
const removeMember = async (req, res) => {
	...
	const { id } = req.body;
	// Call the service
	await boardService.removeMember(boardId, id, (err, result) => {
		if (err) return res.status(400).send(err);
		return res.status(200).send(result);
	});
};

const getBoardMember = async (req, res) => {
	// Validate whether params.id is in the user's boards or not
	const validate = req.user.boards.filter((board) => board === req.params.id);
	if (!validate)
		return res
	...
	const { boardId } = req.params;
	// Call the service
	await boardService.getBoardMembers(boardId, (err, result) => {
		if (err) return res.status(400).send(err);
		return res.status(200).send(result);
	});
};
...
const getMembers = async (req, res) => {
	// Call the service
	await boardService.getMembers((err, result) => {
		if (err) return res.status(400).send(err);
		return res.status(200).send(result);
	});
};
