...
const baseUrl = `${process.env.REACT_APP_APIURL}/board`;
...
export const getMembers = async (fromDropDown, dispatch) => {
  if(!fromDropDown)dispatch(startFetchingBoards());
  try {
    const res = await axios.get(baseUrl + "/members");
    setTimeout(() => {
      dispatch(successFetchingMembers(res.data));
    }, 1000);
  } catch (error) {
    dispatch(failFetchingBoards());
    dispatch(
