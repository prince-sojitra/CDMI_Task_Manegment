...
const baseUrl = `${process.env.REACT_APP_APIURL}/list`;
...
export const updateListOrder = async (props, dispatch) => {
	...
	// because of consistency between server and client.
	let checkIndex = props.allLists.findIndex(elt => elt.title.trim() === `${props.userInfo.name} ${props.userInfo.surname}`)
	if (checkIndex === props.destinationIndex) {
		return false
	}
