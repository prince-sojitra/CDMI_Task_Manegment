...
const listRoute = `${process.env.REACT_APP_APIURL}/list`;
const boardRoute = `${process.env.REACT_APP_APIURL}/board`;

export const getLists = async (boardId, dispatch, userInfo) => {
	dispatch(setLoading(true));
	try {
		const res = await axios.get(listRoute + '/' + boardId);

		const manageAllList = res.data && res.data.length ? [...res.data] : []

		if (manageAllList && manageAllList.length) {
			let compareName = manageAllList.findIndex(elt => elt.title.trim() === `${userInfo.name} ${userInfo.surname}`)
			if (compareName > 0) {
				let copyEl = { ...manageAllList[compareName] }
				manageAllList.splice(compareName, 1)
				manageAllList.splice(0, 0, copyEl)
			}
		}

		dispatch(successFetchingLists(manageAllList));
		setTimeout(() => {
...
export const boardMemberRemove= async (boardId, id, dispatch) => {
	try {
		const result = await axios.post(`${boardRoute}/${boardId}/remove-member`, {
			id
		});
		await dispatch(addMembers(result.data));
		dispatch(
			openAlert({
				message: 'Member is remove to this board successfully',
				severity: 'success',
			})
		);
	} catch (error) {
